			      ...
			      const int32_t LUT[CORDIC_30B_ITABLE_SIZE], cordic_cfg type)
{
	 ...
	 */
	if (b_cosvalue < cord_arcsincos_q28fl) {
		x = 0;
	...
	for (b_i = 0; b_i < i; b_i++) {
		...
		if (x == b_cosvalue) {
			...
			sign = (((x > b_cosvalue) && (y >= 0)) ||
				((x < b_cosvalue) && (y < 0))) ? 1 : -1;
			x = (x - xdshift) - sign * yshift;
			y = (y - ydshift) + sign * xshift;
			z += sign * LUT[b_i];
		}
		b_cosvalue += b_cosvalue >> j;
	}
	if (z < 0)
		z = -z;
			      ...
			      const int32_t LUT[CORDIC_30B_ITABLE_SIZE], cordic_cfg type)
{
	 ...
	 * near the edge of the domain of convergence
	 */
	if (b_sinvalue > cord_arcsincos_q28fl) {
		x = 0;
		y = cord_arcsincos_q30fl;
		z = PI_DIV2_Q3_29;
	} else {
	...
	for (b_i = 0; b_i < i; b_i++) {
		...
		if (y == b_sinvalue) {
			...
			sign = (((y >= b_sinvalue) && (x >= 0)) ||
				((y < b_sinvalue) && (x < 0))) ? 1 : -1;
			x = (x - xdshift) + sign * yshift;
			y = (y - ydshift) - sign * xshift;
			z -= sign * LUT[b_i];
		}
	b_sinvalue += b_sinvalue >> j;
	}
	if (z < 0)
		z = -z;
